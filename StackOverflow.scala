...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(
    ...
    val quesRDD: RDD[(Int, Posting)] =
      postings.filter(x => x.postingType == 1).map(x => (x.id, x))
    val ansRDD: RDD[(Int, Posting)] = postings
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    scored
      .map(x =>
        (firstLangInTag(x._1.tags, langs).getOrElse(0) * langSpread, x._2)
      )
      .persist()
  ...
  @tailrec final def kmeans(
    ...
    val newClusters =
      allClusters.map(x => (x._1, averageVectors(x._2))).collect()
  ...
  def mostCommonLanguage(vectorVal: Iterable[(Int, Int)]): (Int, Int) = {
    ...
    def innerFunc(
        innerVectorVal: Iterable[(Int, Int)],
        langCount: Array[Int]
    ): (Int, Int) = {
  ...
  def clusterResults(
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langPercent: Double =
        langIndex._2 * 100d / clusterSize // percent of the questions in the most common language
