...
object TimeUsage extends TimeUsageInterface:
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def startsWith(str: String, as: String*): Boolean = {
      as.foldLeft(false)((b, a) => b || str.startsWith(a))
    }
    
    @tailrec
    def go(columnNames: List[String], primary: List[Column], working: List[Column], leisure: List[Column])
    : (List[Column], List[Column], List[Column]) = {
      columnNames match {
        case Nil => (primary, working, leisure)
        case x:: xs =>
          if(startsWith(x, "t01", "t03", "t11", "t1801", "t1803"))
            go(xs, col(x)::primary, working, leisure)
          else if (startsWith(x, "t05", "t1805"))
            go(xs, primary, col(x):: working, leisure)
          else if(startsWith(x,  "t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18"))
            go(xs, primary, working, col(x):: leisure)
          else
            go(xs, primary, working, leisure)
      }
    }
    
    go(columnNames, List(), List(), List())
  }
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = (primaryNeedsColumns
      .foldLeft(lit(0.0))((f, col) => f + df(col.toString))/60.0)
      .as("primaryNeeds")
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame =
    summed.select($"sex", $"working", $"age", $"primaryNeeds", $"work", $"other")
